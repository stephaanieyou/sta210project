...
data_range <- max(music_filter$song.tempo, na.rm = TRUE) - min(music_filter$song.tempo, na.rm = TRUE)
width <- data_range/sqrt(nrow(music_filter))
width


ggplot(music_filter, aes(x = song.tempo)) +
  geom_histogram(binwidth = 6, fill = "blue", color = "black") +
  labs(x = "Song Tempo", y = "Count",
       title = "Most songs are 100-150 BPM (allegro)")
